...
然后执行下面命令进行配置：
```
./configure --with-linux-dir=/usr/src/linux-headers-4.9.51-xenomai-3.0.5-aufs --with-module-dir=/lib/modules/4.9.51-xenomai-3.0.5-aufs --enable-generic=no --enable-cycles --enable-hrtimer --enable-8139too=no --enable-e1000e=yes  --with-e1000e-kernel=4.4 --prefix=/usr/local/etherlab
```
